...
void init()
{
	...
	// 加到painter中
	painter->addMesh(disk, "mesh_b", "./assets/disk.jpg", vshader, fshader); 	// 指定纹理与着色器
	meshList.push_back(disk);
 
 	TriMesh* cone = new TriMesh();
	// @TODO: Task2 生成圆锥并贴图
	cone->generateDisk(100, 0.1);
	// 设置物体的旋转位移
	cone->setTranslation(glm::vec3(0.5, 0.0, 0.0));
	cone->setRotation(glm::vec3(0.0, 0.0, 0.0));
	...
	// 加到painter中
	painter->addMesh(cone, "mesh_c", "./assets/cone.jpg", vshader, fshader); 	// 指定纹理与着色器
